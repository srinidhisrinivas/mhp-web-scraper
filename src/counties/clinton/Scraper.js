	...
	this.scrapeByPropertyURL = async function(page, propertyURL){
		...
		for(visitAttemptCount = 0; visitAttemptCount < CONFIG.DEV_CONFIG.MAX_VISIT_ATTEMPTS; visitAttemptCount++){
			try{
				...
				// console.log(e);
				try{
					await page.waitForSelector("table#ContentPlaceHolder1_Base_fvDataOwnerAddress", {timeout: CONFIG.DEV_CONFIG.PARCEL_TIMEOUT_MSEC});
					await page.waitFor(200);
					const ownerTableData = await this.getTableDataBySelector(page, "table#ContentPlaceHolder1_Base_fvDataOwnerAddress > tbody > tr > td > table.formview > tbody > tr",false);
		...
		let ownerTableData = await this.getTableDataBySelector(page, "table#ContentPlaceHolder1_Base_fvDataOwnerAddress > tbody > tr > td > table.formview > tbody > tr",false);
