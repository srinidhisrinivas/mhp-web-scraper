...
let Scraper = function(){
	...
	this.scrapeByPropertyURL = async function(page, propertyURL){
		...
		for(visitAttemptCount = 0; visitAttemptCount < CONFIG.DEV_CONFIG.MAX_VISIT_ATTEMPTS; visitAttemptCount++){
			try{
				...
				try{
					...
					await page.waitFor(200);
					const ownerTableData = await this.getTableDataBySelector(page, "table#ContentPlaceHolder1_Base_fvDataProfileOwner > tbody > tr > td > table.formview > tbody > tr",false);
		...
		let ownerTableData = await this.getTableDataBySelector(page, "table#ContentPlaceHolder1_Base_fvDataProfileOwner > tbody > tr > td > table.formview > tbody > tr",false);
			...
			try{
				...
				try{
					...
					for(let i = 0; i < sideMenu.length; i++){
						...
						let prop = await handle.getProperty('innerText');
						let propJSON = await prop.jsonValue();
						if(propJSON === 'Real Estate Parcel') transferTag = handle;
					}
					await transferTag.click();
